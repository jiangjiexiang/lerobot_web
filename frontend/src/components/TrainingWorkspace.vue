    ...
    <div class="training-layout">
      ...
      <section class="job-detail" aria-label="训练任务详情">
        <template v-if="selectedJob">
          ...
          <div class="job-actions"><button class="secondary" title="复制为新的草稿任务" @click="cloneJob(selectedJob.id)">复制任务</button><button v-if="!selectedJob.archivedAt && !['running','stopping'].includes(selectedJob.state)" class="secondary" title="从默认任务列表隐藏" @click="archiveJob(selectedJob.id, true)">归档</button><button v-if="selectedJob.archivedAt" class="secondary" @click="archiveJob(selectedJob.id, false)">恢复</button><button v-if="selectedJob.state === 'draft'" :disabled="runningJobs > 0 || checking" @click="startJob(selectedJob.id)">▶ 启动训练</button><button v-if="['failed','cancelled'].includes(selectedJob.state) && selectedJob.checkpoints.length" :disabled="runningJobs > 0" @click="resumeJob(selectedJob.id)">↻ 从 Checkpoint 续训</button><button v-if="selectedJob.state === 'running'" class="stop" @click="stopJob(selectedJob.id)">■ 停止</button></div>
