  ...
  <main class="training-workspace">
    ...
    <div v-if="workspaceMode === 'training'" class="training-layout">
      <div class="training-toolbar">
        <div class="queue-summary"><span><strong>{{ runningJobs }}</strong> 运行中</span><span><strong>{{ jobs.length }}</strong> 个任务</span><span><strong>{{ completedJobs }}</strong> 已完成</span></div>
        <button class="new-job-button" @click="showComposer = !showComposer">{{ showComposer ? "收起配置" : "+ 新建任务" }}</button>
      </div>
      <section v-if="showComposer" class="config-pane" aria-label="新建训练任务">
        <div class="pane-heading"><div><h3>新建任务</h3></div><button class="close-composer" title="收起新建任务" @click="showComposer = false">×</button></div>
        <div class="form-grid">
      ...
      <section class="jobs-pane" aria-label="训练任务列表">
        <div class="pane-heading"><div><h3>任务队列</h3></div><label class="archive-toggle"><input v-model="showArchived" type="checkbox" @change="refreshJobs" /> 含归档</label></div>
        <div class="job-filters"><input v-model.trim="jobSearch" type="search" placeholder="搜索任务或数据集" /><select v-model="jobFilter"><option value="all">全部状态</option><option value="active">运行中</option><option value="draft">待启动</option><option value="completed">已完成</option><option value="failed">失败或停止</option></select></div>
        ...
        <div v-else class="empty">没有符合条件的任务</div>
      </section>

      <section :class="['job-detail', `view-${detailView}`]" aria-label="训练任务详情">
        <template v-if="comparisonJobs.length >= 2">
          <div class="job-title"><div><h3>实验对比</h3></div><button class="close-compare" @click="compareIds = []">退出对比</button></div>
        ...
        <template v-else-if="selectedJob">
          <div class="job-title"><div><h3>{{ selectedJob.name }}</h3></div><span :class="['job-state', selectedJob.state]">{{ jobState(selectedJob.state) }}</span></div>
          <nav class="detail-tabs" aria-label="任务详情视图"><button :class="{ active: detailView === 'overview' }" @click="detailView = 'overview'">概览</button><button :class="{ active: detailView === 'metrics' }" @click="detailView = 'metrics'">指标</button><button :class="{ active: detailView === 'artifacts' }" @click="detailView = 'artifacts'">产物与日志</button></nav>
          ...
          <div class="job-actions"><button v-if="selectedJob.state === 'completed'" :class="['secondary', { best: selectedJob.bestAt }]" @click="markBest(selectedJob.id, !selectedJob.bestAt)">{{ selectedJob.bestAt ? "★ 最佳 Run" : "标记最佳" }}</button><button class="secondary" title="复制为新的草稿任务" @click="cloneJob(selectedJob.id)">复制任务</button><button class="secondary" :disabled="cloudSpecLoading" @click="loadCloudSpec">{{ cloudSpecLoading ? "生成云端方案…" : "导出云端方案" }}</button><button v-if="!selectedJob.archivedAt && !['running','stopping'].includes(selectedJob.state)" class="secondary" title="从默认任务列表隐藏" @click="archiveJob(selectedJob.id, true)">归档</button><button v-if="selectedJob.archivedAt" class="secondary" @click="archiveJob(selectedJob.id, false)">恢复</button><button v-if="selectedJob.state === 'draft'" :disabled="runningJobs > 0 || checking" @click="startJob(selectedJob.id)">▶ 启动训练</button><button v-if="['failed','cancelled'].includes(selectedJob.state) && selectedJob.checkpoints.length" :disabled="runningJobs > 0" @click="resumeJob(selectedJob.id)">↻ 从 Checkpoint 续训</button><button v-if="selectedJob.state === 'running'" class="stop" @click="stopJob(selectedJob.id)">■ 停止</button></div>
          <section v-if="cloudSpec" class="cloud-spec"><div><strong>云端训练方案 · Generic SSH</strong><button @click="downloadCloudSpec">下载 JSON</button></div><p>同步 {{ cloudSpec.dataset.remoteRoot }} · 资源估算显存 {{ cloudSpec.resourceEstimate.gpuMemoryGb }} GB / 内存 {{ cloudSpec.resourceEstimate.systemMemoryGb }} GB</p><code>{{ cloudSpec.remote.trainCommand.join(" ") }}</code><small>仅生成可审计命令，不连接云主机、不上传数据。</small></section>
          ...
          <div v-else-if="detailView === 'metrics'" class="detail-empty">任务启动并写入指标后，将在这里显示训练曲线。</div>
...
interface CloudSpec { schemaVersion: number; provider: string; generatedAt: string; job: { id: string; name: string; policy: string; device: string; batchSize: number; steps: number; numWorkers: number; seed: number }; dataset: { name: string; collection: string; episodes: number[]; localRoot: string; remoteRoot: string; manifestUrl: string }; resourceEstimate: { gpuMemoryGb: number; systemMemoryGb: number; basis: string }; sync: { exclude: string[]; command: string[] }; remote: { workingDirectory: string; trainCommand: string[]; installHint: string }; notes: string[] }
...
const detailView = ref<"overview" | "metrics" | "artifacts">("overview");
const showComposer = ref(false);
const jobSearch = ref("");
const jobFilter = ref<"all" | "active" | "draft" | "completed" | "failed">("all");
...
const cloudSpec = ref<CloudSpec | null>(null);
const cloudSpecLoading = ref(false);
...
const completedJobs = computed(() => jobs.value.filter((job) => job.state === "completed").length);
const filteredJobs = computed(() => {
  const query = jobSearch.value.toLowerCase();
  return jobs.value.filter((job) => {
    const matchesQuery = !query || `${job.name} ${job.dataset} ${job.collection} ${job.policy}`.toLowerCase().includes(query);
    const matchesState = jobFilter.value === "all"
      || (jobFilter.value === "active" && ["running", "stopping"].includes(job.state))
      || (jobFilter.value === "failed" && ["failed", "cancelled"].includes(job.state))
      || job.state === jobFilter.value;
...
async function loadCloudSpec() { const job = selectedJob.value; if (!job || cloudSpecLoading.value) return; cloudSpecLoading.value = true; try { const data = await requestJson(`/api/training/jobs/${encodeURIComponent(job.id)}/cloud-spec`); if (selectedJob.value?.id === job.id) cloudSpec.value = data.spec as CloudSpec; } catch (cause) { error.value = cause instanceof Error ? cause.message : String(cause); } finally { cloudSpecLoading.value = false; } }
function downloadCloudSpec() { if (!cloudSpec.value) return; const blob = new Blob([JSON.stringify(cloudSpec.value, null, 2)], { type: "application/json" }); const url = URL.createObjectURL(blob); const anchor = document.createElement("a"); anchor.href = url; anchor.download = `${cloudSpec.value.job.id}-cloud-spec.json`; anchor.click(); URL.revokeObjectURL(url); }
async function createJob() { if (!formValid.value || creating.value) return; creating.value = true; error.value = null; try { const data = await requestJson("/api/training/jobs", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) }); selectedJobId.value = data.job.id; detailView.value = "overview"; showComposer.value = false; await refreshJobs(); } catch (cause) { error.value = cause instanceof Error ? cause.message : String(cause); } finally { creating.value = false; } }
...
watch(selectedJobId, () => { preflight.value = null; snapshot.value = null; cloudSpec.value = null; Object.keys(checkpointHashes).forEach((key) => delete checkpointHashes[key]); void loadDatasetSnapshot(); });
...
.training-layout { display: grid; grid-template-columns: 320px minmax(0,1fr); grid-template-areas: "toolbar toolbar" "composer composer" "jobs detail"; min-height: calc(100vh - 225px); border: 1px solid rgba(151,188,222,.15); border-top: 0; background: #0d1928; }.training-toolbar { grid-area: toolbar; display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 9px 12px; border-bottom: 1px solid rgba(151,188,222,.13); background: #0a1622; }.queue-summary { display: flex; flex-wrap: wrap; gap: 15px; color: #6f8799; font-size: 8px; }.queue-summary strong { margin-right: 3px; color: #c8d9e4; font: 11px ui-monospace,monospace; }.new-job-button { padding: 7px 10px; border: 1px solid #35708d; border-radius: 4px; cursor: pointer; background: #14354a; color: #9bdaf0; font-size: 9px; }.config-pane { grid-area: composer; padding: 14px; border-bottom: 1px solid rgba(151,188,222,.13); background: #0c1a28; }.jobs-pane { grid-area: jobs; border-right: 1px solid rgba(151,188,222,.13); }.job-detail { grid-area: detail; }.config-pane,.jobs-pane,.job-detail { min-width: 0; padding: 14px; }.pane-heading,.job-title { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; margin-bottom: 14px; }.pane-heading h3,.job-title h3 { margin-top: 2px; font-size: 13px; }.pane-heading > span { color: #70899c; font-size: 8px; }.close-composer { border: 0; cursor: pointer; background: transparent; color: #7891a5; font-size: 18px; }
.form-grid { display: grid; grid-template-columns: repeat(4,minmax(130px,1fr)); gap: 9px; }.form-grid label { display: grid; gap: 4px; color: #8ca2b3; font-size: 9px; }.form-grid input,.form-grid select { width: 100%; min-width: 0; padding: 8px; border: 1px solid #29435f; border-radius: 4px; outline: 0; background: #091522; color: #e7edf8; font-size: 10px; }.form-grid input:focus,.form-grid select:focus { border-color: #5a8bad; }.config-warning { margin-top: 9px; padding: 7px 8px; border-left: 2px solid #e4b85d; background: #282218; color: #d8b970; font-size: 8px; line-height: 1.5; }.quality-summary { margin-top: 9px; border: 1px solid rgba(84,201,148,.25); background: #10251f; }.quality-summary.warning { border-color: rgba(225,191,118,.35); background: #282218; }.quality-summary.fail { border-color: rgba(232,121,138,.45); background: #25141b; }.quality-summary > div { display: flex; justify-content: space-between; padding: 7px 8px; border-bottom: 1px solid rgba(151,188,222,.1); color: #83a498; font-size: 8px; }.quality-summary.fail > div { color: #f39aa7; }.quality-summary button { border: 0; cursor: pointer; background: transparent; color: #76cdec; font-size: 8px; }.quality-summary p { margin: 0; padding: 7px 8px; color: #6f9b8a; font-size: 8px; }.quality-summary.warning p { color: #d0b56f; }.quality-summary.fail p { color: #f39aa7; }.resource-estimate { margin-top: 9px; border: 1px solid rgba(84,201,148,.25); background: #10251f; }.resource-estimate.fail { border-color: rgba(232,121,138,.45); background: #25141b; }.resource-estimate > div { display: flex; justify-content: space-between; padding: 7px 8px; border-bottom: 1px solid rgba(151,188,222,.1); color: #83a498; font-size: 8px; }.resource-estimate.fail > div { color: #f39aa7; }.resource-estimate p { margin: 0; padding: 7px 8px; color: #6f9b8a; font-size: 8px; line-height: 1.5; }.resource-estimate.fail p { color: #f39aa7; }.training-flags { display: flex; flex-wrap: wrap; gap: 4px; margin-top: 10px; }.training-flags span { padding: 3px 5px; border-radius: 3px; background: #142638; color: #6f899c; font-size: 8px; }.create-job { width: auto; min-width: 150px; margin: 12px 0 0 auto; padding: 9px 16px; border: 0; border-radius: 4px; cursor: pointer; background: #64c9e8; color: #07131c; font-size: 9px; font-weight: 650; }.create-job:disabled { cursor: not-allowed; opacity: .45; }
...
.job-filters { display: grid; grid-template-columns: 1fr 92px; gap: 6px; margin-bottom: 10px; }.job-filters input,.job-filters select { width: 100%; min-width: 0; padding: 7px; border: 1px solid #29435f; border-radius: 3px; outline: 0; background: #091522; color: #9fb3c1; font-size: 8px; }
...
.detail-tabs { display: flex; gap: 2px; margin: -5px 0 12px; border-bottom: 1px solid rgba(151,188,222,.13); }.detail-tabs button { padding: 7px 11px; border: 0; border-bottom: 2px solid transparent; cursor: pointer; background: transparent; color: #71899b; font-size: 9px; }.detail-tabs button.active { border-bottom-color: #64c9e8; color: #b8d8e8; }.job-detail.view-overview .metrics-panel,.job-detail.view-overview .checkpoint-panel,.job-detail.view-overview .model-panel,.job-detail.view-overview .command,.job-detail.view-overview .logs { display: none; }.job-detail.view-metrics > .job-facts,.job-detail.view-metrics > .progress,.job-detail.view-metrics > dl,.job-detail.view-metrics > .dataset-snapshot,.job-detail.view-metrics > .job-actions,.job-detail.view-metrics > .cloud-spec,.job-detail.view-metrics > .preflight-panel,.job-detail.view-metrics > .checkpoint-panel,.job-detail.view-metrics > .model-panel,.job-detail.view-metrics > .command,.job-detail.view-metrics > .logs { display: none; }.job-detail.view-artifacts > .job-facts,.job-detail.view-artifacts > .progress,.job-detail.view-artifacts > dl,.job-detail.view-artifacts > .dataset-snapshot,.job-detail.view-artifacts > .cloud-spec,.job-detail.view-artifacts > .preflight-panel,.job-detail.view-artifacts > .metrics-panel { display: none; }
...
.cloud-spec { margin-top: 9px; border: 1px solid rgba(118,205,236,.25); background: #0c202d; }.cloud-spec > div { display: flex; justify-content: space-between; padding: 7px 9px; border-bottom: 1px solid rgba(151,188,222,.1); color: #9ac8d8; font-size: 8px; }.cloud-spec button { border: 0; cursor: pointer; background: transparent; color: #76cdec; font-size: 8px; }.cloud-spec p,.cloud-spec small { display: block; margin: 0; padding: 7px 9px 0; color: #7295a5; font-size: 8px; line-height: 1.5; }.cloud-spec code { display: block; max-height: 80px; overflow: auto; padding: 7px 9px; color: #a7d5e3; font: 8px/1.5 ui-monospace,monospace; white-space: pre-wrap; overflow-wrap: anywhere; }.cloud-spec small { padding-top: 0; padding-bottom: 8px; color: #698294; }
