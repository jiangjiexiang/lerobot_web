...
- 评估 Episode 单独记录成功、失败、无效和失败原因，同时保留数据质量审核；无效数据不进入模型成功率分母。
- 模型登记聚合所有关联评估任务的成功率，并返回 Wilson 95% 置信区间，避免小样本结果被误读。
- 每个训练任务绑定的 Dataset/Collection 可生成可复现 snapshot manifest，记录选集内容、所有原始文件相对路径、大小、修改时间、SHA-256 和整体 fingerprint；原始 Parquet/视频只读，`.lerobot-web` sidecar 不进入快照。
- Dataset 质量扫描抽样检查视频可读性、黑屏、低亮度和冻结帧，并检查状态/动作非有限值、轨迹突变与首中尾签名重复风险；扫描结果进入训练启动前预检，错误风险阻止训练，警告保留人工确认空间。
- 只有 production 模型可正常部署；部署前校验受管目录、配置文件、模型文件和 SHA-256，随后原子切换本机模型目录指针，不启动推理或驱动机械臂。
- 每次部署和回滚都创建不可变修订，保留模型版本、产物哈希、操作人和备注；回滚同样执行完整产物校验并生成新修订。
- 部署工作区展示当前版本、指针健康、评估摘要和历史修订；推理抖动、偏移、碰撞、相机、延迟及硬件异常可关联当前修订记录并闭环处理。
- 发布门禁默认要求至少 10 个有效评估 Episode、成功率不低于 80%、Wilson 95% 区间下界不低于 50%，并阻止模型自身未关闭的错误/严重异常；阈值可审计调整，受控放行必须填写理由并记录规则快照。
- 部署目标提供只读运行遥测：推理进程通过心跳上报延迟与成功结果，平台聚合推理次数、错误率、平均/P95 延迟；错误或延迟过高显示降级，超过 30 秒无心跳自动显示离线，心跳写入按目标串行化避免并发丢计数。

## 后续训练闭环

- 按任务成功率、运行稳定性和人工审核结果选择生产版本。
- 增加推理服务启停、运行指标和受控灰度发布；涉及机械臂动作时必须由操作员显式确认。
